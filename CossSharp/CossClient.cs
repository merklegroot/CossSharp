using CossSharp.Http;
using CossSharp.Models;
using CossSharp.Signator;
...
namespace CossSharp
{
    ...
    public class CossClient : ICossClient
    {
        ...
        public string GetOrderBookRaw(string baseSymbol, string quoteSymbol)
        {
            if (string.IsNullOrWhiteSpace(baseSymbol)) { throw new ArgumentNullException(nameof(baseSymbol)); }
            if (string.IsNullOrWhiteSpace(quoteSymbol)) { throw new ArgumentNullException(nameof(quoteSymbol)); }

            var tradingPairText = GetTradingPairSymbol(baseSymbol, quoteSymbol);
        ...
        public CossOrderBook GetOrderBook(string baseSymbol, string quoteSymbol)
             => GetAndDeserialize<CossOrderBook>(() => GetOrderBookRaw(baseSymbol, quoteSymbol));
        ...
        public string GetOpenOrdersRaw(CossApiKey apiKey, string baseSymbol, string quoteSymbol)
        {
            const string Path = "order/list/open";

            return AuthenticatedPost(TradeRoot, apiKey, Path, () =>
            {
                var cossSymbol = $"{baseSymbol}-{quoteSymbol}".ToLower();
        ...
        public CossGetOpenOrdersResponseMessage GetOpenOrders(CossApiKey apiKey, string baseSymbol, string quoteSymbol)
        {
            var contents = GetOpenOrdersRaw(apiKey, baseSymbol, quoteSymbol);
            return JsonConvert.DeserializeObject<CossGetOpenOrdersResponseMessage>(contents);
        }

        public string CreateOrderRaw(CossApiKey apiKey, string baseSymbol, string quoteSymbol, decimal price, decimal quantity, bool isBid)
        {
            const string Path = "/order/add";

            var tradingPairSymbol = GetTradingPairSymbol(baseSymbol, quoteSymbol);
        ...
        public CossCreateOrderResponseMessage CreateOrder(CossApiKey apiKey, string symbol, string quoteSymbol, decimal price, decimal quantity, bool isBid)
        {
            var contents = CreateOrderRaw(apiKey, symbol, quoteSymbol, price, quantity, isBid);
            return contents != null
                ? JsonConvert.DeserializeObject<CossCreateOrderResponseMessage>(contents)
                : null;
        }

        public string CancelOrderRaw(CossApiKey apiKey, string baseSymbol, string quoteSymbol, string orderId)
        {
            ...
            return AuthenticatedPost(TradeRoot, apiKey, Path, () =>
            {
                var combo = $"{baseSymbol}_{quoteSymbol}".ToUpper();
        ...
        public CossCancelOrderResponseMessage CancelOrder(CossApiKey apiKey, string baseSymbol, string quoteSymbol, string orderId)
            => GetAndDeserialize<CossCancelOrderResponseMessage>(() => CancelOrderRaw(apiKey, baseSymbol, quoteSymbol, orderId));

        public string GetCompletedOrdersRaw(CossApiKey apiKey, string baseSymbol, string quoteSymbol, int? limit = null, int? page = null)
        {
            const string Path = "order/list/completed";

            return AuthenticatedPost(TradeRoot, apiKey, Path, () =>
            {
                var combo = $"{baseSymbol}_{quoteSymbol}".ToUpperInvariant();
        ...
        public CossGetCompletedOrdersResponseMessage GetCompletedOrders(CossApiKey apiKey, string baseSymbol, string quoteSymbol, int? limit = null, int? page = null)
        {
            var contents = GetCompletedOrdersRaw(apiKey, baseSymbol, quoteSymbol, limit, page);
        ...
        private string GetTradingPairSymbol(string baseSymbol, string quoteSymbol)
        {
            if (string.IsNullOrWhiteSpace(baseSymbol)) { throw new ArgumentNullException(nameof(baseSymbol)); }
            if (string.IsNullOrWhiteSpace(quoteSymbol)) { throw new ArgumentNullException(nameof(quoteSymbol)); }

            return $"{baseSymbol.Trim().ToUpper()}_{quoteSymbol.Trim().ToUpper()}";
        }
